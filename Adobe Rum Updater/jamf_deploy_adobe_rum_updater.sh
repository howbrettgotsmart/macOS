...
LOG_DIR="/var/log/AdobeRUMUpdater"
SCRIPT_PATH="${INSTALL_DIR}/adobe-rum-updater.sh"
PLIST_LABEL="com.company.adobe-rum-updater"
PLIST_PATH="/Library/LaunchDaemons/${PLIST_LABEL}.plist"

if [[ "${EUID}" -ne 0 ]]; then
  echo "This script must run as root. Jamf Pro policies run scripts as root by default."
  exit 1
fi

/bin/mkdir -p "${INSTALL_DIR}" "${LOG_DIR}"
/usr/sbin/chown root:wheel "${INSTALL_DIR}" "${LOG_DIR}"
/bin/chmod 755 "${INSTALL_DIR}" "${LOG_DIR}"

/bin/cat >"${SCRIPT_PATH}" <<'__ADOBE_RUM_UPDATER_SCRIPT__'
#!/bin/bash
set -u
...
main "$@"
__ADOBE_RUM_UPDATER_SCRIPT__

/usr/sbin/chown root:wheel "${SCRIPT_PATH}"
/bin/chmod 755 "${SCRIPT_PATH}"

/bin/cat >"${PLIST_PATH}" <<'__ADOBE_RUM_UPDATER_PLIST__'
...
<dict>
  <key>Label</key>
  <string>com.company.adobe-rum-updater</string>

  <key>ProgramArguments</key>
  <array>
    <string>/Library/Management/AdobeRUMUpdater/adobe-rum-updater.sh</string>
  </array>
  ...
  <string>/var/log/AdobeRUMUpdater/launchd.out.log</string>

  <key>StandardErrorPath</key>
  <string>/var/log/AdobeRUMUpdater/launchd.err.log</string>
</dict>
</plist>
__ADOBE_RUM_UPDATER_PLIST__

/usr/sbin/chown root:wheel "${PLIST_PATH}"
/bin/chmod 644 "${PLIST_PATH}"

if ! /usr/bin/plutil -lint "${PLIST_PATH}" >/dev/null; then
  echo "LaunchDaemon plist failed validation: ${PLIST_PATH}"
  exit 1
fi
...
echo "Installed updater script: ${SCRIPT_PATH}"
echo "Installed LaunchDaemon: ${PLIST_PATH}"
echo "To test: sudo launchctl kickstart -k system/${PLIST_LABEL}"
